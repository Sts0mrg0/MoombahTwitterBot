...
class TwitterBot:
    ...
    def auto_fav(self, phrase, logger=None, count=100, result_type="recent"):
        """
        ...
        for tweet in result["statuses"]:
            ...
            # when you have already favorited a tweet, this error is thrown
                ...
                if "status 429" in str(api_error).lower():
                    if logger:
                        logger.error("auto_fav: You have been rate limited.")
                    else:
    ...
    def auto_rt(self, phrase, logger=None, count=100, result_type="recent"):
        """
        ...
        for tweet in result["statuses"]:
            ...
            # when you have already retweeted a tweet, this error is thrown
                ...
                if "status 429" in str(api_error).lower():
                    if logger:
                        logger.error("auto_rt: You have been rate limited.")
                    else:
                        print("auto_rt - You have been rate limited.",file=sys.stderr)
                    return
    ...
    def auto_follow(self, phrase, logger=None, count=100, result_type="recent"):
        """
        ...
        for tweet in result["statuses"]:
            try:
                if (tweet["user"]["screen_name"] != self.BOT_CONFIG["TWITTER_HANDLE"] and
                    ...
                    if logger:
                        logger.info("[--]Followed %s" % (tweet["user"]["screen_name"]))
                    else:
                        print("Followed %s" % (tweet["user"]["screen_name"]), file=sys.stdout)

            except TwitterHTTPError as api_error:
                # quit on rate limit errors
                if "status 429" in str(api_error).lower():
                    if logger:
                        logger.error("auto_Follow: You have been rate limited.")
                    else:
                        print("auto_Follow - You have been rate limited.",file=sys.stderr)
                    return
    ...
    def auto_follow_followers(self,count=None,logger=None):
        ...
        for user_id in not_following_back:
            try:
                ...
                if "status 429" in str(api_error).lower():
                    if logger:
                        logger.error("auto_follow_followers: You have been rate limited.")
                    else:
                        print("auto_follow_followers: You have been rate limited.", file=sys.stderr)
                    return
                else:
                    if "already requested to follow" not in str(api_error).lower():
                        if logger:
    ...
    def auto_follow_followers_of_user(self, user_twitter_handle, count=100):
        ...
        for user_id in followers_of_user:
            try:
                ...
                # quit on rate limit errors
                if ("unable to follow more people at this time" in str(api_error).lower()) or ("status 429" in str(api_error).lower()):
